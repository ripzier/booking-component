...
const Passengers = ({className}) => {
  ...
  const focused = focusedInput === 'passengers' ? 'focused' : ''
  ...
  return (
    <div
      className={`relative bg-white border border-gray-light ${focused} ${className}`}
    >
      ...
      {focused && (
        <div className='absolute left-0 right-0 border-t  border-gray-light bg-white'>
          <PassengerInput
            type='adults'
            range='12+'
            value={adults}
          ...
          />
          <button
            className='text-center text-lg hover:text-primary py-4 w-full'
            onClick={changeFocus}
          >
            Close
          </button>
